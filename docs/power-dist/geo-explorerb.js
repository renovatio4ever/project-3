...
!(function(undefined) {
    ...
    function init(){
      ...
      map = L.mapbox.map('app-maparea', 'ARMsio-io.kae20cg0', {
      ...
      map.attributionControl.addAttribution('<a href=\'https://doble.com/\'>BRMS Analytics</a>');
      ARMsioMapData = L.layerGroup().addTo(map);
    ...
    function getParams(selector) {
      var params = ARMsio.utils.parseParams(document.location.search);
    ...
    function updateQuery() {
      ...
      str = '?';
      ARMsio.utils.each(params, function(value, key){
        str += key + '=' + value + '&';
      });
    ...
    // ARMsio.utils.each(queries, function(q){});
    ...
    function draw(){
      ...
      if (lat && lng && rad) {
        geoFilter.push({
          property_name : 'ARMsio.location.coordinates',
          operator : 'within',
          property_value: {
      ...
      var scoped_events = new ARMsio.Query('select_unique', {
        event_collection: 'user_action',
        target_property: 'ARMsio.location.coordinates',
      ...
      client.run(scoped_events, function(err, res){
        ...
        ARMsio.utils.each(res.result, function(coord, index){
      ...
      var hearts = new ARMsio.Query('median', {
        event_collection: 'user_action',
        interval: 'daily',
      ...
      var daily_median_heartrate = new ARMsio.SensorData()
        .el('#chart-01')
        .height(300)
        .colors(['#fe6672'])
      ...
      var activations = new ARMsio.Query('count', {
        event_collection: 'activations',
        interval: 'daily',
        timeframe: baseParams.timeframe,
        filters: baseParams.filters
      });
      var daily_activations = new ARMsio.SensorData()
        .el('#chart-02')
        .height(300)
        .colors(['#5a9eed'])
        .title('Daily Circuit Trips')
        .type('area')
        .prepare();
      ...
      var purchases = new ARMsio.Query('sum', {
      ...
      var daily_purchases = new ARMsio.SensorData()
        .el('#chart-03')
        .height(300)
        .colors(['#eeb058'])
        .title('Peak Power Factor')
        .type('area')
        .prepare();
